...
impl Nextcloud {
	...
	fn send_message_once (& self, message: String, chat: String) -> Result<reqwest::Response, reqwest::Error> {
		...
		let result = reqwest::Client::new()
			.post(&format!("{}/ocs/v2.php/apps/spreed/api/v1/chat/{}", &self.url, chat))
			.basic_auth(&self.user, self.pass.as_ref())
			.headers(headers)
			.body(format!("{{\"token\": \"{}\", \"message\": \"{}\"}}", chat, message))
			.send();
		match result {
			Ok(response) => {
	...
	pub fn send_message (& self, message : String) {
		...
		match result {
			Ok(_response) => (),
			Err(old_error) => {
				let result = self.send_message_once (message.to_string() + " (sent again)", self.chat.to_string());
				match result {
					Ok(_response) => (),
					Err(error) => {
						eprintln!("Couldn't send {} first because {} and then because {}", message, old_error, error);
					},
				}
			}
		}
	...
	fn update_status (&mut self) {
		self.set_status(format!("{} {} {}", self.info_online, self.info_door, self.info_environment));
	}
	...
	fn set_status (& self, message: String) {
		...
		let result = reqwest::Client::new()
			.put(&format!("{}/ocs/v2.php/apps/user_status/api/v1/user_status/message/custom", &self.url))
			.basic_auth(&self.user, self.pass.as_ref())
			.headers(headers)
