...
impl Validator {
	pub fn new(config: &mut Config) -> Self {
		Self { users: config.get_hash_map_vec_u8("validator"), timeout: 0 }
	}
...
mod tests {
	...
	fn test_validate() {
		...
		assert_eq!(validator.timeout, 0);
		assert_eq!(validator.validate(&mut vec![14, 15, 13, 15, 13, 15, 11, 15, 7]), Validation::None);
		assert_eq!(validator.timeout, 1);
		assert_eq!(validator.validate(&mut vec![14, 15, 13, 15, 11, 15, 7, 15]), Validation::Validated("1234".to_string()));
	}
}
